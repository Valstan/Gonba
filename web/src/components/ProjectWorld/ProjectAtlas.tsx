...
import { projectCoverImage } from './projectImage'
  ...
  return (
    <div className="project-atlas" role="list">
      {projects.map((project, index) => {
        ...
        const image = projectCoverImage(project) || projectCoverArt(project)
        const label =
        ...
        return (
          ...
          >
            ...
            <div className="project-atlas__content">
              <span className="project-atlas__eyebrow">
                {projectEyebrow(project).split(' · ')[0]}
              </span>
              <h2>{label}</h2>
              <p>{projectNarrative(project)}</p>
              <span className="project-atlas__enter">
                Войти в проект <b aria-hidden>↗</b>
              </span>
            </div>
          </Link>
        )
