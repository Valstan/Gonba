 ...
 *
 * Сколько это по времени — зависит от РАСПИСАНИЯ, а не от самого числа, и это
 * связь, которую легко не заметить при следующей правке (pool #289: повторы и
 * период запуска надо пересчитывать вместе). При слотах 06/10/13/18/23 три
 * попытки покрывают 7–10 часов подряд — достаточно, чтобы пережить транзиентный
 * сбой, и достаточно мало, чтобы очередь не стояла сутками.
 *
 * Прежнее обоснование гласило «примерно сутки, потому что гард съедает каждый
 * второй прогон». Гард починен, прогоны больше не теряются — то есть число
 * осталось прежним, а его смысл изменился. Меняешь расписание — перечитай эту
 * оценку, она не самоочевидна из константы.
 */
...
export async function syncVkSource(
  ...
  try {
    ...
    //
    // Гард защищает от частых вызовов РУЧНОГО триггера; расписание задаёт
    // таймер. Без допуска эти две роли дрались, и вот как именно.
    //
    // `lastSyncAt` пишется в КОНЦЕ прогона, то есть на несколько секунд позже
    // срабатывания таймера. Значит промежуток, который видит гард, всегда
    // короче промежутка между срабатываниями на длительность прогона. Пока у
    // таймера стоял `RandomizedDelaySec=5min`, срабатывания «плавали» в пределах
    // пяти минут вокруг слота, и всякий раз, когда следующее срабатывание
    // приходилось на меньший сдвиг, чем предыдущее, промежуток проваливался
    // ниже трёх часов и прогон отбрасывался целиком.
    //
    // Замер по журналу за сутки (08.09 12:01 — 09.09 15:04): **2 отброшенных из
    // 10 срабатываний**, оба ровно по этому сценарию — 18:03:30 → 21:03:15
    // (2 ч 59 мин 45 с) и 00:03:27 → 03:03:24 (2 ч 59 мин 57 с). Прежняя оценка
    // «съедает каждый второй прогон» была выведена из одного наблюдения и
    // завышена: доля зависит от того, как легли две случайные задержки подряд.
    //
    // Допуск нужен и после снятия разброса, и это не перестраховка: при точных
    // слотах промежуток 10:00→13:00 равен ровно трём часам, а `lastSyncAt` от
    // прогона в 10:00 отстоит на секунды вперёд — без допуска этот слот
    // отбрасывался бы КАЖДЫЙ раз, уже детерминированно. Пять минут заведомо
    // перекрывают длительность прогона и заведомо меньше самого узкого
    // промежутка расписания. Ручной триггер допуск не развязывает: 5 минут
    // против часов интервала.
    const GUARD_TOLERANCE_MS = 5 * 60 * 1000
    if (sourceDoc.lastSyncAt) {
      ...
      if (now - lastSync < intervalMs - GUARD_TOLERANCE_MS) {
