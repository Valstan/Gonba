...
export async function syncVkSource(
  ...
  try {
    ...
    // таймер. Пока допуска не было, эти две роли дрались: `lastSyncAt`
    // пишется в КОНЦЕ прогона, поэтому промежуток между двумя срабатываниями
    // таймера всегда на длительность прогона МЕНЬШЕ его периода. При интервале,
    // равном промежутку слотов, гард не мог быть удовлетворён в принципе —
    // это не гонка, а систематический недобор. Замер 09.09: таймер отработал
    // в 15:04, `lastSyncAt` у всех шести источников остался 12:04, не хватило
    // ~55 секунд, прогон отброшен целиком.
    //
    // Допуск в 5 минут — с запасом в сто раз над наблюдавшимся недобором и
    // заведомо меньше самого узкого промежутка расписания (10:00→13:00).
    // Ручной триггер он по-прежнему сдерживает: 5 минут против часов.
