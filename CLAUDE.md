...
3. **Релиз** — `/reliz` ведёт через commit → push → PR → merge → build:raw через systemd-run → restart gonba → проверки. Один шаг = один диалог.
4. **Закрытие сессии** — перед последним коммитом обнови `DEVELOPMENT_LOG.md` (новый блок сверху, в дату сессии) и закрой/перенеси задачи в `PENDING_FOLLOWUPS.md`.
