...
Отдельно от старения (#033, шаг 2.1 — «лежит давно») у пунктов `PENDING_FOLLOWUPS.md` может стоять **жёсткая дата** `due: YYYY-MM-DD` (формат — в шапке файла, секция «Метка срока»). Пройдись по открытым (не ✅) пунктам с `due:` и сравни с сегодняшней датой (`# currentDate`). **Всплыви в отчёте** — отдельным блоком **«⏰ Сроки»**, выше блокеров:

- `due` ≤ сегодня → **🔴 Срок наступил** — предложи взять в работу сейчас (или явно перенести `due`, не молча).
- `due` в пределах **14 дней** → **🟡 Срок близко (N дней)** — предупреди, чтобы успеть запланировать.
- `due` дальше 14 дней → молчать.

Если подходящих нет — блок не выводить. Это **активная защита** для дата-привязанных обязанностей (квартальный самоосмотр #036, ramp/cleanup фичефлага, ETA внешней задачи), которую система старения не ловит: пункт может быть «свежим» по `added`, но дата уже наступила. Жёсткие сроки дополнительно подстрахованы `/schedule`-рутинами (сработают даже без открытия сессии) — этот шаг ловит их при любой сессии на/после даты.
