---
name: "source-command-reliz"
description: "Релизный flow GONBA — commit → push → PR → merge → safe-build на проде → restart → проверки."
---

# source-command-reliz

Use this skill when the user asks to run the migrated source command `reliz`.
...
## Шаг 2. Обновить `PENDING_FOLLOWUPS.md`

**Это критично** — не пропускать.

- Открой `docs/PENDING_FOLLOWUPS.md`. Если что-то из закрываемой задачи висело в ⏳/🟡/🟢 — отметь `✅ Сделано` (с PR/коммитом) или удали. Если в процессе вылезли новые техдолги — допиши их.
- **Changelog релиза = тело PR** (Summary + Test plan). Отдельного журнала нет — хронология живёт в `git log` + PR ([ADR-0007](../../docs/adr/0007-archive-development-log.md)). Поэтому тело PR пиши содержательно: что меняли, почему, как применять на проде.

Сделай это **перед** коммитом, чтобы PENDING попал в тот же коммит.
...
3. `git commit -m "$(heredoc)"` с подробным телом + `Co-Authored-By: Codex Opus 4.7 (1M context) <noreply@anthropic.com>`
