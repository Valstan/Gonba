...
|---|---|---|
| `brain → GONBA` | brain | `brain_matrica/mailboxes/GONBA/from-brain/*.md` (мы только **читаем** локально и через GitHub API; чужой репо не синхронизируем) |
| `GONBA → brain` | GONBA | **`mailbox/to-brain/*.md`** в этом репо (коммитим в свой через PR) |

### Шаги в начале каждой сессии

1. **Sync только GONBA:** `fetch` / `pull --ff-only` разрешены только в текущем репозитории. Любые соседние репо, включая `../brain_matrica/`, не синхронизировать и не менять (`fetch` / `pull` / `checkout` запрещены).
2. **Сканить входящий mailbox двухканально** (только корень — **не** `DRAFTS/`, **не** `ARCHIVE/`):
   - локально: `../brain_matrica/mailboxes/GONBA/from-brain/*.md`;
   - GitHub `main`: тот же путь через `gh api` / web, без `clone` / `fetch` / `pull` в `brain_matrica`.
   Набор писем — объединение источников по имени файла. Если одноимённые версии различаются: незакоммиченная локальная версия считается более свежим кандидатом; иначе сравнить последний локальный коммит **этого файла** с последним коммитом **этого пути** на GitHub. Если порядок не определяется — прочитать обе версии, явно отметить конфликт и ничего не перезаписывать. Свежесть одного письма или репозитория нельзя переносить на другое письмо или репозиторий.
...
Закоммитить **в свой репо через PR** (отдельным PR или вместе с тематическим). Brain прочитает письмо из своего двухканального mailbox-скана.
...
### Sibling-репо: read-only чтение напрямую (ADR-0007 мозга, 2026-07-05)

Разрешено **читать любой sibling-репо** (`../<project>/`) **read-only напрямую**, но запрещено выполнять в нём `fetch`, `pull`, `checkout` и другие синхронизирующие или изменяющие команды: API-контракты, `docs/`, `SESSION_HANDOFF.md`, `mailbox/to-brain/` соседа читаются в том состоянии, которое уже лежит на диске. Для актуальности mailbox используй отдельный GitHub API/web-канал без изменения клона. Эвристика: «нужно ЗНАТЬ про соседа → читай сам; нужно чтобы сосед что-то СДЕЛАЛ/ЗАПОМНИЛ → письмо в мозг». Записи/коммиты в чужой репо и peer-to-peer директивы — по-прежнему запрещены (ADR-0001 v3). **Если построил зависимость от чужого API/формата** (не разовое чтение, а интеграция) — сообщить мозгу письмом, чтобы связь попала в реестр.
...
Нашёл релевантное — переиспользую опыт (и при желании отпишусь в `mailbox/to-brain/`, что применил). Не нашёл — продолжаю как обычно. **Тишина = норма:** триггер не сработал за сессию → ноль лишних чтений. Локальный brain-клон уже осматривается на `/start` без синхронизации; актуальность входящей почты отдельно проверяется через GitHub API. См. [pool #014](../brain_matrica/cross-project-ideas/ideas/014-consult-library-reflex.md).
...
- ❌ Выполнять `clone` / `fetch` / `pull` / `checkout` или любые записи в `brain_matrica` и других sibling-репо — они только для чтения; актуальность mailbox проверять через GitHub API/web
...
- **`gh pr merge`** — авто-мерж под гейтами (см. «Автономия под гейтами» ниже): сливаем зелёный PR (CI `web-quality` required check) без отдельного «окей», когда локальные гейты (typecheck/lint) и CI зелёные. Человеческий OK остаётся обязателен только для прод-операций с данными (#025).
...
- **Прод-данные деструктив** — всегда `AskUserQuestion` в том же ходе, режим `auto` это не отменяет (#025).

**Что в `deny` (жёсткий блок):** `git push origin main`, force-push в `main`, `gh workflow run deploy*` (ручной деплой-dispatch). Force-push в feature-ветку — разрешён (см. выше).
...
### Публичный репозиторий — recon-поверхность (D-038, mandate brain 2026-08-23)

**Правило — через свойство, а не через канал:** хостнеймы, IP, порты, пути на сервере, имена SSH-ключей и алиасов, состав жильцов общего бокса и имя хостера **не кладём в отслеживаемые файлы вообще**. Не «не публикуем на витрине», не «не выносим на сайт» — **не в git**.

Формулировка важнее, чем кажется. Прежний предохранитель у нас был написан и соблюдался — но написан был **про витрину**, про то, что уезжает на публичный сайт. Пока репозиторий приватный, разница между «на сайте» и «в документах репозитория» имела смысл. **17 августа 2026 репозиторий стал публичным, и разница исчезла, а тексты остались прежними.** Правило, сформулированное через канал, перестаёт защищать при смене канала — **не выглядя нарушенным**: следующий агент впишет в доку имя бокса и будет формально прав. Это [#189](../brain_matrica/cross-project-ideas/ideas/189-a-ban-written-about-one-channel-does-not-survive-the-channel-change.md).

**Чужая инфраструктура — тем более.** Адрес шлюза соседнего проекта, имена его CI-ключей, состав общего бокса: это не наше, чтобы публиковать. Именно на этом класс и поймали (Портфолио выложило прод-IP чужого проекта кликабельной ссылкой).

**Где тогда живут значения:** прод-хост, порт и пользователь — в `~/.ssh/config` каждой dev-машины под алиасом `GONBA`; на новой машине берутся у владельца. Все инструкции в доках пишутся через алиас (`ssh GONBA "..."`) — тогда конкретный адрес не нужен ни одному примеру.

**Явно записанное исключение** (иначе правило разойдётся с практикой и его перестанут читать): пути на сервере (`/etc/gonba/…`, каталог релизов) остаются в отслеживаемых файлах там, где **функционально необходимы рабочему файлу** — systemd-юнит `deploy/systemd/gonba.service`, `deploy-prod.yml`, `scripts/safe-build.sh` без них не работают. Вычищать их из прозы, пока юнит публикует те же строки рядом, — театр, а не защита. Исключение действует только на функциональную необходимость: в описательный текст пути класть незачем.

**Историю не переписываем и исторические записи не правим** — отправленные письма в `mailbox/to-brain/`, `docs/SESSION_HANDOFF.md`, закрытые ✅-пункты в `PENDING_FOLLOWUPS.md`. Это записи о том, что было; переписывать их нечестно, а отозвать всё равно нечего.

⚠️ **Честная граница:** вычистка **ничего не отзывает**. Всё вычищенное лежит в истории с первых коммитов и стало публичным вместе с ней; форки, клоны и кэши не отозвать. Владелец взвесил и решил адреса не менять и фаервол не ставить — значит остаточный риск принят сознательно. Это гигиена на будущее, и делать вид, что риск закрыт, не надо.
