...
- 🟡 **Единая медиа: вынести кэш из `public/` + разовый слив legacy** (нашли 2026-06-03 при аудите ротации). Схема работает (Я.Диск primary, горячее на VPS, daily-таймер ротации крутится), и legacy `public/media` заведён в ту же atime-ротацию ([#96](https://github.com/Valstan/Gonba/pull/96), на проде). **Нюанс:** `media-cache` и `media` лежат внутри `public/`, который задевается деплой-сборкой → atime как «сигнал спроса» приблизителен (на проде atime наблюдался шумным, dry-run сейчас вымывает 0). Чистая схема: (а) `MEDIA_CACHE_DIR=/var/lib/gonba/media-cache` (вне `public/`, правка env + `gonba-media-cache.service` уже читает `EnvironmentFile`); (б) разово удалить 363 локальных копии в `public/media` (все имеют `yandexPath` → дотянутся по спросу) + ~33 orphan-файла (нет записи в БД, proxy не отдаёт — закрывает старый orphan-пункт ниже). Прод-операция (env + systemd + удаление файлов), под OK владельца. Не блокер — #96 безвреден, выметет холодное когда появится.
