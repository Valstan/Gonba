...
- **Yandex Disk UI визуальный обход со скриншотами**: PR #5/#6 + фаза D (микро-чистка hardcoded shadows) убрали базовые расхождения, но полный визуальный QA (выравнивание, размеры бейджей, mobile) требует открыть `/admin/yadisk` глазами и пройтись по элементам. Не блокер.
