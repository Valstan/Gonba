...
- 🟡 **Deploy guard по содержимому (stale ISR-prerender).** 2026-05-30 деплой PR #55 прошёл «успешно» (CI зелёный, build success, smoke-checks HTTP 200), но `/` отдавал **старый prerender** (этно-страница вместо карусели) — Next.js переиспользовал устаревший `.next/server/app/index.html`, несмотря на `rm -rf .next` внутри `safe-build.sh`. Лечилось ручной чистой пересборкой (явный `rm -rf .next` + verify). **Что сделать:** (а) в `.github/workflows/deploy-prod.yml` smoke-check проверять *маркер* нового контента на `/` (grep по ответу), не только код 200; (б) в `scripts/safe-build.sh` после `rm -rf .next` добавить `[ -d .next ] && exit 1`. Урок и детали — DEV_LOG 2026-05-30; шеринг — `mailbox/to-brain/2026-05-30-orbit-raf-and-stale-prerender.md`.
