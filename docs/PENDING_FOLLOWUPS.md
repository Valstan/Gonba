...
- 🟡 **Медиа-библиотека: дедуп / слияние дублей / связи / безопасное удаление** — план-памятка `docs/plans/media-library-integrity.md`. Phase A (выбор существующей картинки) **✅ сделано**; Phase B (дедуп при загрузке по `yandexSha256`) **✅ сделано 2026-06-01** (`web/src/utilities/mediaUpload.ts` → `uploadOrReuseMedia`; проверено 2026-06-03 — единственный raw `POST /api/media` идёт через этот хелпер, все редакторы подключены через `InlineImage`/`MediaPicker` → дедуп универсален по сайт-инструментам; не покрыт только стандартный `/admin`-аплоад, это серверный дедуп = Phase D). **Следующий настоящий этап — Phase C** (usage-связи «где используется» + safe-delete с заменой): крупный/ядро, карта полей включая rich-text, тестировать на копии прод-БД перед удалением. Затем D (слияние дублей). Не блокер.
