...
## 2026-05-31 — ГОНЬБА 31 мая 2026 (Claude session) — Stale-prerender bites again: главная → force-dynamic (root-cause найден)

**Тема:** после деплоя v2 (#62) прод **отдавал старую версию карусели** — несмотря на «успешный» деплой (CI зелёный, smoke-checks 200, anti-stale-guard прошёл). Тот же класс бага, что 2026-05-30, но в этот раз **по CSS**.

### Диагностика (root-cause найден, наконец)

- Прод-build `6861028b3b422ba4.css` (109KB) **содержит** новый `orbit-spin` + `23vw` + `.homeOrbit__ring`. Build корректный.
- НО пререндеренный `.next/server/app/index.html` (mtime = этого билда, 05:02) ссылается на **фантомный** CSS-хэш `438f4eaae6bbebdf.css`, которого **нет на диске** ни в одном чанке/манифесте. `app-build-manifest.json` при этом ссылается на **правильный** `6861028b`.
- Т.е. **build-манифест верный, а статический prerender `/` — устаревший** (ссылается на CSS-бандл прошлого билда). `safe-build.sh` сделал чистый `rm -rf .next` (guard прошёл) — но prerender всё равно вышел stale. Это и есть та «загадка 2026-05-30, почему чистая пересборка не всегда помогает».
- Anti-stale deploy-guard НЕ поймал: он проверяет маркер `homeOrbit__itemWrap`, который есть и в старой, и в новой версии (маркер версионно-агностичен).
- Локальный прод-эндпоинт (127.0.0.1:3000, мимо nginx) тоже отдавал stale → не CDN-кэш, а сам Next-prerender.

### Фикс (PR fix/home-force-dynamic)

- **`web/src/app/(frontend)/page.tsx`** — `export const revalidate = 600` → **`export const dynamic = 'force-dynamic'`**. Главная больше **не пререндерится** на build'е: рендерится по текущему манифесту запущенного сервера на каждый запрос → всегда свежие контент + CSS. Цена — 1 запрос к БД на просмотр главной (для этого сайта приемлемо). Stale-prerender для `/` теперь невозможен в принципе.

### Урок — переносимо (→ pool brain)

- **«HTTP 200 + наличие версионно-агностичного маркера» ≠ свежий деплой.** Next.js (App Router, ISR `revalidate`) может оставить **устаревший статический prerender маршрута** даже при чистом `rm -rf .next` — `index.html` ссылается на CSS-бандл прошлого билда, хотя `app-build-manifest.json` уже новый. **Надёжная защита для критичных маршрутов, меняющихся между деплоями: `force-dynamic`** (или анти-stale guard, проверяющий *актуальный CSS-хэш из app-build-manifest присутствует в ответе*, а не просто маркер компонента). Маркер должен быть **версионным** (например, хэш сборки), иначе guard слеп к CSS/контент-дрейфу.

---

## 2026-05-31 — ГОНЬБА 31 мая 2026 (Claude session) — Орбит-карусель v2: фикс вращения (CSS keyframes) + компактность

**Тема:** правки по фидбэку владельца на v1 (PR #61): (1) карусель **не крутилась** (а до рефактора #55 — крутилась в том же браузере); (2) кружки **слишком разлетелись** — свести компактно, почти впритык, но подписи не должны наезжать.

### Что сделано (PR feat/orbit-spin-fix-compact)

- **Фикс вращения → CSS keyframes.** Вернул `animation: orbit-spin 90s linear infinite` на `.homeOrbit__ring` и `orbit-counter-spin 90s` на `.homeOrbit__itemInner` (keyframes уже были в `globals.css`). Убрал rAF + `transform: rotate(var(--orbit-rot))` из `HomeCarouselMenuClient.tsx`. Пауза на hover/focus — через CSS (`:hover/:focus-within → animation-play-state: paused`), `prefers-reduced-motion` guard сохранён. Компонент стал **серверным** (убран `'use client'` + хуки — чистая презентационная разметка).
- **Компактность.** `--orbit-radius` clamp(17rem,28vw,24rem) → **clamp(16rem,23vw,19.5rem)** (radius ≈ 1.45×диаметр). Дуга-подпись прижата ближе к кружку (arc radius 58→55, font 11→10, textLength 108→100). Stage `min-height` уменьшен. Результат (численно на прототипе): зазор между кружками **~20px** (было ~69px), клиренс подписей до соседей **+13px** (не наезжают).

### Урок (root-cause «не крутится») — переносимо

- **rAF, обновляющий `transform: rotate(var(--orbit-rot))` через незарегистрированную CSS-переменную, может НЕ перерисовывать transform** в браузере (баг рефактора #55: визуально не крутилось). Диагностика: старая версия (#55~) крутилась CSS-анимацией `orbit-spin` **и** имела `prefers-reduced-motion`-guard, но у пользователя крутилась ⇒ reduced-motion у него выключен ⇒ дело не в guard, а в rAF+var-механизме. **Надёжно — либо CSS `@keyframes`, либо зарегистрировать `@property --orbit-rot { syntax:'<angle>' }`** (тогда var в transform анимируется). Выбрал CSS keyframes (проще, доказано работает). Дрейф двух анимаций (исходная причина рефактора на rAF) убирается одинаковой длительностью spin/counter-spin (90s = 90s) и отсутствием конфликтующего media-query (он и был источником дрожания — 75s против 140s). → кандидат в pool brain.

### Как верифицировано

- Прототип + SVG glyph-API: зазор кружков ~20px, клиренс подписей +13px при tightest-вьюпорте (worst case). `typecheck` чистый.
- ⚠️ **Само вращение в этом окружении не наблюдаемо** (headless preview не продвигает ни rAF, ни CSS-анимации — `getComputedStyle(transform)` статичен). Доверяю эмпирике пользователя (CSS keyframes крутились) — финальная проверка вращения на проде/в браузере.
- Локальный dev рендер `/` упал **500 = local Postgres OOM** на drizzle-интроспекции (известная флакки dev-БД, не код) — прод-build на здоровой БД.
